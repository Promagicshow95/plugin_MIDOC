...
global {
    ...
    action analyser_trips {
        ...
        // Utiliser une approche alternative : analyser directement les données GTFS
        // En se basant sur les shapes existantes et en vérifiant si elles sont référencées
        
        // Méthode simplifiée : considérer qu'une shape est utilisée si elle existe dans le GTFS
        // (car les shapes inutilisées ne seraient normalement pas incluses)
        ask transport_shape {
            // Pour cette version simplifiée, on considère toutes les shapes comme potentiellement utilisées
            // sauf si on peut prouver le contraire par d'autres moyens
            
            // Ici on peut implémenter une logique plus simple :
            // Si une shape a été créée depuis le GTFS, elle est probablement utilisée
            if (shapeId != nil) {
