...
experiment pontualite_Toulouse type: gui {
    output {
        ...
        display "Monitoring" {
            chart "Performance Temporelle" type: series {
                data "Avance (s)" value: mean(bus collect mean(each.arrival_time_diffs_pos)) color: #green;
                data "Retard (s)" value: mean(bus collect mean(each.arrival_time_diffs_neg)) color: #red;
            }
        }
