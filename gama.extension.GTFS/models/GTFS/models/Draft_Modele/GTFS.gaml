...
//          " = " + current_seconds_mod + " secondes";
		...
		current_seconds_abs <- int(cycle * step / #s);
...
//		write "current_seconds_abs: " + current_seconds_abs;



		if (current_seconds_abs >= int(departure_time)){
...
species bus skills: [moving] {
	...
	reflex wait_at_stop when: waiting_at_stop {
		...
		if (current_seconds_abs >= stop_time) {
...
//}
    ...
    if (current_stop_index < length(departureStopsInfo) - 1) {
        ...
        int actual_time <- current_seconds_abs;
