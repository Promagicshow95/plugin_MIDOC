 ...
 */
@skill(name = "TransportStopSkillNonDelegate", doc = @doc("Skill for agents that manage public transport stops based on GTFS data. It allows loading GTFS stops and creating corresponding agents."))
@vars({
    @variable(name = "stops", 
        type = IType.LIST, 
        of = IType.AGENT, 
    ...
    @variable (
			name = "stopId",
			type = IType.STRING,
			doc = @doc ("Identify of a stop")),
    @variable (
			name = "stopName",
			type = IType.STRING,
			doc = @doc ("Name of a stop")),
    
      
})

public class TransportStopSkillNonDelegate extends Skill {
    ...
    public int loadStopsFromGTFS(final IScope scope) {
        ...
        try {
			...
			IPopulation<IAgent> stopPopulation = (IPopulation<IAgent>) scope.getSimulation().getPopulationFor("bus_stop");
